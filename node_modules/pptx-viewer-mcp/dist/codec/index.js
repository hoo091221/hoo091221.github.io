...
var SCALAR_ELEMENT_KEYS = new Set(
  Object.entries(pptxViewerCore.ELEMENT_FIELD_KIND).filter(([, kind]) => kind === "scalar" || kind === "asset").map(([field]) => field)
);
var COMPLEX_FIELD_MAP = Object.fromEntries(
  Object.entries(pptxViewerCore.ELEMENT_FIELD_KIND).filter(([, kind]) => kind === "complex").map(([field]) => [field, `_${field}`])
);
...
var SCALAR_SLIDE_KEYS = new Set(
  Object.entries(pptxViewerCore.SLIDE_FIELD_KIND).filter(([, kind]) => kind === "scalar").map(([field]) => field)
);
var COMPLEX_SLIDE_FIELD_MAP = Object.fromEntries(
  Object.entries(pptxViewerCore.SLIDE_FIELD_KIND).filter(([, kind]) => kind === "complex").map(([field]) => [field, `_${field}`])
);
...
var PptxCodec = class {
  ...
  async hydrate(ydoc, bytes, origin) {
    ...
    ydoc.transact(() => {
      ...
      const sourceBytesArr = new yjs.Array();
  ...
  _slideToYMap(slide) {
    ...
    const elementsArray = new yjs.Array();
  ...
  _yMapToElement(elemMap) {
    ...
    elemMap.forEach((value, key) => {
      if (key === "textBody" && value instanceof yjs.Text) {
...
exports.COMPLEX_FIELD_MAP = COMPLEX_FIELD_MAP;
exports.COMPLEX_SLIDE_FIELD_MAP = COMPLEX_SLIDE_FIELD_MAP;
exports.ORIGIN_FILE_LOAD = ORIGIN_FILE_LOAD;
exports.PptxCodec = PptxCodec;
exports.SCALAR_ELEMENT_KEYS = SCALAR_ELEMENT_KEYS;
exports.SCALAR_SLIDE_KEYS = SCALAR_SLIDE_KEYS;
