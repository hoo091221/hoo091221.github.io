...
var forge3__default = /*#__PURE__*/_interopDefault(forge3);
...
function certFingerprintSha256(certPem) {
  try {
    const cert = new crypto2__default.default.X509Certificate(certPem);
...
async function evaluateTimestampAuthority(signatureXml) {
  try {
    ...
    const asn1 = forge3__default.default.asn1.fromDer(forge3__default.default.util.createBuffer(tokenDer.toString("binary")));
...
function certificateInfoFromBase64(certBase64) {
  try {
    const certificate = new crypto2__default.default.X509Certificate(Buffer.from(certBase64, "base64"));
    return {
...
function validateCertificateChain(certBase64List, additionalRootsPem) {
  ...
  try {
    const chain = certBase64List.map((value) => {
      const der = forge3__default.default.util.decode64(value);
      return forge3__default.default.pki.certificateFromAsn1(forge3__default.default.asn1.fromDer(der));
    });
    const rootPem = [...tls__default.default.rootCertificates, ...additionalRootsPem];
    const caStore = forge3__default.default.pki.createCaStore(rootPem);
    const verified = forge3__default.default.pki.verifyCertificateChain(caStore, chain);
...
function verifySignatureValue(signatureXml, certBase64List) {
  try {
    ...
    const verifier = crypto2__default.default.createVerify(verifyAlgorithm);
...
function loadSigningMaterialFromBuffer(certificateBuffer, certificatePath, certificatePassword) {
  ...
  if (extension === ".pfx" || extension === ".p12") {
    const p12Der = forge3__default.default.util.createBuffer(Buffer.from(certificateBuffer).toString("binary"));
    const p12Asn1 = forge3__default.default.asn1.fromDer(p12Der);
    const p12 = forge3__default.default.pkcs12.pkcs12FromAsn1(p12Asn1, certificatePassword || "");
    const keyBag = p12.getBags({
      bagType: forge3__default.default.pki.oids.pkcs8ShroudedKeyBag
    })[forge3__default.default.pki.oids.pkcs8ShroudedKeyBag];
    const certBag = p12.getBags({ bagType: forge3__default.default.pki.oids.certBag })[forge3__default.default.pki.oids.certBag];
    ...
    return {
      privateKeyPem: forge3__default.default.pki.privateKeyToPem(keyBag[0].key),
      certificatePem: forge3__default.default.pki.certificateToPem(certBag[0].cert)
    };
  }
...
function buildOcspRequestDer(leafPem, issuerPem) {
  try {
    const leaf = forge3__default.default.pki.certificateFromPem(leafPem);
    const issuer = forge3__default.default.pki.certificateFromPem(issuerPem);
    const issuerNameDer = forge3__default.default.asn1.toDer(forge3__default.default.pki.distinguishedNameToAsn1(issuer.subject)).getBytes();
    const issuerNameHash = forge3__default.default.md.sha1.create().update(issuerNameDer).digest().getBytes();
    const pubKeyDer = forge3__default.default.asn1.toDer(forge3__default.default.pki.publicKeyToAsn1(issuer.publicKey)).getBytes();
    const pubKeyAsn1 = forge3__default.default.asn1.fromDer(pubKeyDer);
    const bitString = asn1Child2(pubKeyAsn1, 1);
    const rawKey = bitString ? asn1Bytes(bitString).substring(1) : "";
    const issuerKeyHash = forge3__default.default.md.sha1.create().update(rawKey).digest().getBytes();
    const serialBytes = forge3__default.default.util.hexToBytes(leaf.serialNumber);
    const request = forge3__default.default.asn1.create(forge3__default.default.asn1.Class.UNIVERSAL, forge3__default.default.asn1.Type.SEQUENCE, true, [
      forge3__default.default.asn1.create(forge3__default.default.asn1.Class.UNIVERSAL, forge3__default.default.asn1.Type.SEQUENCE, true, [
        forge3__default.default.asn1.create(forge3__default.default.asn1.Class.UNIVERSAL, forge3__default.default.asn1.Type.SEQUENCE, true, [
          forge3__default.default.asn1.create(forge3__default.default.asn1.Class.UNIVERSAL, forge3__default.default.asn1.Type.SEQUENCE, true, [
            forge3__default.default.asn1.create(forge3__default.default.asn1.Class.UNIVERSAL, forge3__default.default.asn1.Type.SEQUENCE, true, [
              forge3__default.default.asn1.create(forge3__default.default.asn1.Class.UNIVERSAL, forge3__default.default.asn1.Type.SEQUENCE, true, [
                forge3__default.default.asn1.create(
                  forge3__default.default.asn1.Class.UNIVERSAL,
                  forge3__default.default.asn1.Type.OID,
                  false,
                  forge3__default.default.asn1.oidToDer(SHA1_OID).getBytes()
                ),
                forge3__default.default.asn1.create(forge3__default.default.asn1.Class.UNIVERSAL, forge3__default.default.asn1.Type.NULL, false, "")
              ]),
              forge3__default.default.asn1.create(
                forge3__default.default.asn1.Class.UNIVERSAL,
                forge3__default.default.asn1.Type.OCTETSTRING,
                false,
                issuerNameHash
              ),
              forge3__default.default.asn1.create(
              ...
              forge3__default.default.asn1.create(
                forge3__default.default.asn1.Class.UNIVERSAL,
                forge3__default.default.asn1.Type.INTEGER,
                false,
                serialBytes
              )
            ])
    ...
    return Buffer.from(forge3__default.default.asn1.toDer(request).getBytes(), "binary");
  } catch {
    return void 0;
  }
}
function parseOcspResponseStatus(data) {
  try {
    const asn1 = forge3__default.default.asn1.fromDer(forge3__default.default.util.createBuffer(data.toString("binary")));
    ...
    const basicResp = forge3__default.default.asn1.fromDer(forge3__default.default.util.createBuffer(asn1Bytes(respOctet)));
    const tbsData = asn1Child2(basicResp, 0);
    if (!tbsData) {
      return "unknown";
    }
    const first = asn1Child2(tbsData, 0);
    const responsesIdx = first?.tagClass === forge3__default.default.asn1.Class.CONTEXT_SPECIFIC && first.type === 0 ? 3 : 2;
    ...
    if (certStatus.tagClass === forge3__default.default.asn1.Class.CONTEXT_SPECIFIC) {
...
function computeDigestBase642(content, digestAlgorithmUri) {
  ...
  return crypto2__default.default.createHash(hashName).update(Buffer.from(content)).digest("base64");
}
...
async function buildOfficeReferenceList(zip) {
  ...
  for (const entryPath of Object.keys(zip.files)) {
    ...
    const digestValue = crypto2__default.default.createHash("sha256").update(Buffer.from(entryBytes)).digest("base64");
    references.push({
