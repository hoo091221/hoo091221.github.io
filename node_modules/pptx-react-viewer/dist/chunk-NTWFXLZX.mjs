import { SHAPE_PRESET_DEFS, VIEWER_SHORTCUT_REFERENCE, STROKE_DASH_OPTIONS, getAnimationInitialStyle, cloneHistorySnapshot, hasShapeProperties, getImageFitStyle, getComputedFillStyle, isImageLikeElement, getCropShapeClipPath, getImageTilingStyle, mediaPlaybackAttributes, buildTextBlockStyle, hasTextProperties, toCssWritingMode, toCssTextOrientation, toCssVerticalDirection, buildTextBodyLayoutStyle, buildTextBody3DSceneStyle, buildParagraphs, colorWithOpacity, normalizeHexColor, hexToRgbUnit, build3DExtrusionData, getComputedEffectStyle, getComputedStrokeStyle, getAnimationColorBaseStyle, isStrokeOnlyPresetElement, resolveShapeGeometry, normalizeStrokeDashType, getSvgStrokeDasharray, buildSvgGradientDef, buildStrokeOutline, getPresetShapeVectorGeometry, isWedgeCalloutPresetShape, svgGradientFillRef, isCalloutShape, getCalloutLeaderLineGeometry, buildCalloutLeaderLineSvgPath, getCalloutViewBoxBounds, svgLineCap, getCompoundLineOffsets, getCompoundLineWidths, canDrillDown, computeSelectionRect, DEFAULT_FONT_FAMILY, cellStyleToCss, rectToCells, isCellInRect, tableContainerCss, buildChartViewModel, resolveChartKind, chartPreserveAspectRatio, buildNotesPdfBytes, buildSlidesPdfBytes, buildPreviewAnimation, buildMotionPathPreview, mediaFallbackVisual, scheduleMediaTrimAndFade, registerCrossSlideAudio, resolveAutoFitFontScale, resolveFontKerning, getSubstituteFontFamily, parsePanoseString, resolveScriptFontSet, hasDistinctScriptFonts, nestedTextDecorationStyle, pieceLetterSpacing, resolveMetricTrackingPx, hollowTextFillStyle, stripUnderlineDecoration, buildTabContext, TEXT_BUILD_ID_SEP, buildTextBuildSpec, textBuildSpanStyle, apply3dEffects, clampUnitInterval, customGeometryPathsToSvgSubpaths, buildSubpathPaints, computeColumnBoundaries, getTableCellBandStyle, parseDrawingColor, parseDrawingColorOpacity, getCellDiagonalBorders, cellRunStyle, getChartStylePalette, computeChartLegendLayout, mediaFallbackIcon, mediaFallbackLabelKey, MEDIA_CHROME_ATTRIBUTE, convertOmmlToMathMl, sanitizeMathMl, tableCellCss, computeResizedColumnWidths, computeResizedRowHeight, DEFAULT_ROW_HEIGHT, ooxmlGradientAngleToCssDegrees, buildRectPathGradient, buildRunTabLines, chartPartToAttrs, splitWordsForUnderline, splitRunForMetrics, splitRunByScriptFont } from './chunk-VQX7G75Z.mjs';
import { translationsEn } from './chunk-JBSAKP4Z.mjs';
import { clsx } from 'clsx';
import { twMerge } from 'tailwind-merge';
import React7, { useRef, useState, useEffect, useCallback, useMemo, useLayoutEffect } from 'react';
...
function getAnimationInitialStyle2(preset, nativeAnimation) {
  return getAnimationInitialStyle(preset, nativeAnimation);
}
function cn(...inputs) {
...
function cloneHistorySnapshot2(snapshot) {
  return cloneHistorySnapshot(snapshot);
}

// src/viewer/constants/scalar.ts
var DEFAULT_CANVAS_WIDTH = 1280;
var DEFAULT_CANVAS_HEIGHT = 720;
var MIN_ELEMENT_SIZE = 12;
var EMU_PER_PX = 9525;
var DEFAULT_BODY_INSET_TB_PX = 45720 / 9525;
var DEFAULT_TEXT_COLOR = "#111827";
var DEFAULT_FILL_COLOR = "#3b82f6";
var DEFAULT_STROKE_COLOR = "#1f2937";
var DEFAULT_FONT_FAMILY2 = '"Segoe UI", "Helvetica Neue", Arial, sans-serif';
var DEFAULT_TEXT_FONT_SIZE = 24;
...
var SHORTCUT_REFERENCE_ITEMS = VIEWER_SHORTCUT_REFERENCE;

// src/viewer/constants/transitions-animations.ts
var ANIMATION_PRESET_OPTIONS = [
...
var SHAPE_PRESETS = SHAPE_PRESET_DEFS.map(
  (def) => ({
    type: def.type,
    label: def.label,
    i18nKey: def.i18nKey,
    icon: React7.createElement(GLYPH_COMPONENTS[def.glyph], {
      className: def.glyphClass ? `w-3.5 h-3.5 ${def.glyphClass}` : "w-3.5 h-3.5"
    })
  })
);

// src/viewer/constants/connectors-strokes.ts
var CONNECTOR_ARROW_OPTIONS = [
...
var STROKE_DASH_OPTIONS2 = STROKE_DASH_OPTIONS;

// src/viewer/utils/shape-types.tsx
...
// src/viewer/utils/element.ts
function isConnectorOrLineElement(element) {
...
function getImageMaskStyle(element) {
  ...
  return geometry.kind === "clipPath" ? { clipPath: geometry.clipPath } : void 0;
}
...
function getImageSurfaceMaskStyle(element) {
  const mask = getImageMaskStyle(element);
  return {
    overflow: "hidden",
    borderRadius: mask?.borderRadius,
    clipPath: mask?.clipPath ?? getCropShapeClipPath2(element)
  };
}
function getImageSurfaceStyle(element) {
  const fill = hasShapeProperties(element) ? getComputedFillStyle(element) : void 0;
  return {
    position: "absolute",
    inset: 0,
...
function getCropShapeClipPath2(element) {
...
function shouldRenderFallbackLabel(element, isTextElement) {
  if (isTextElement) {
    return false;
  }
  ...
  if (element.type === "ole") {
    const previewData = element.previewImageData ?? element.previewImage;
    return !previewData;
  }
...
function buildTrimFragment(element) {
  ...
  return `#t=${(start / 1e3).toFixed(3)}`;
}
function useMediaMetadataExtraction(mediaRef, element) {
  ...
  useEffect(() => {
    ...
    if (element.metadata?.duration !== void 0) {
...
function CaptionTrackRenderer({
  captionTracks
}) {
  return /* @__PURE__ */ jsx(Fragment, { children: captionTracks.map((track) => {
    const trackSrc = track.src ?? (track.content ? `data:text/vtt;charset=utf-8,${encodeURIComponent(track.content)}` : void 0);
    if (!trackSrc) {
      return null;
    }
    return /* @__PURE__ */ jsx(
...
function VideoWithMetadata({
  ...
  return /* @__PURE__ */ jsxs(
    "video",
    {
      ...
      loop: shouldLoop,
      onPlay,
      crossOrigin: captionTracks.length > 0 ? "anonymous" : void 0,
      children: [
        /* @__PURE__ */ jsx("source", { src: `${dataUrl}${trimFragment}`, type: mediaMimeType || "video/mp4" }),
        captionTracks.length > 0 && /* @__PURE__ */ jsx(CaptionTrackRenderer, { captionTracks })
      ]
    }
  );
}
function AudioWithMetadata({
  element,
  mediaRef,
  onPlay,
  dataUrl,
  ...
  useMediaMetadataExtraction(mediaRef, element);
  return /* @__PURE__ */ jsx("div", { className: "w-full h-full flex items-center justify-center p-2 pointer-events-auto", children: /* @__PURE__ */ jsx(
    "audio",
    {
      ref: mediaRef,
      className: "w-full",
      ...
      loop: shouldLoop,
      onPlay,
      children: /* @__PURE__ */ jsx("source", { src: `${dataUrl}${trimFragment}`, type: mediaMimeType || "audio/mpeg" })
    }
  ) });
}
function PresentationMediaController({
  ...
  const noopOnPlay = useCallback(() => {
  }, []);
  return /* @__PURE__ */ jsxs("div", { className: "w-full h-full", style: wrapperStyle, children: [
    children({ mediaRef, onPlay: noopOnPlay, isMediaPlaying }),
    isFullScreen && isPresentationMode && isMediaPlaying && /* @__PURE__ */ jsx(
      "button",
      {
        type: "button",
        ...
        children: /* @__PURE__ */ jsx("svg", { width: "18", height: "18", viewBox: "0 0 24 24", fill: "currentColor", stroke: "none", children: /* @__PURE__ */ jsx("rect", { x: "6", y: "6", width: "12", height: "12", rx: "1" }) })
      }
    )
  ] });
}
function label(key) {
  return key === void 0 ? void 0 : translationsEn[key];
}
function FallbackIcon({
  ...
  return /* @__PURE__ */ jsx(
    "svg",
    {
      ...
      strokeWidth: "1.5",
      className,
      children: paths.map((d) => /* @__PURE__ */ jsx("path", { d }, d))
    }
  );
}
function Badge({
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      children: [
        ...
        isMissing && /* @__PURE__ */ jsx("span", { className: "text-[10px] text-white/60", children: label(mediaFallbackLabelKey(visual, element.mediaType)) })
      ]
    }
  );
}
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsx("span", { className: `text-[10px] ${isMissing ? "text-white/50" : "text-white/70"}`, children: label(mediaFallbackLabelKey(visual, element.mediaType)) })
      ]
    }
  );
}
  ...
  if (visual.poster && posterUrl) {
    return /* @__PURE__ */ jsxs("div", { className: "w-full h-full relative pointer-events-none", children: [
      /* @__PURE__ */ jsx(
        ...
        {
          src: posterUrl,
          alt: visual.dimPoster ? translationsEn["pptx.media.posterAlt"] : translationsEn["pptx.media.videoPosterAlt"],
          className: `w-full h-full${visual.dimPoster ? " opacity-50" : ""}`,
          style: getImageFitStyle(element)
        }
      ),
      /* @__PURE__ */ jsx(Badge, { visual, element })
    ] });
  }
  return /* @__PURE__ */ jsx(Placeholder, { visual, element });
}
function renderMediaElement(element, mediaDataUrls, options) {
  if (element.type !== "media") {
    return /* @__PURE__ */ jsx("div", { className: "w-full h-full flex items-center justify-center text-[11px] text-white/80 pointer-events-none", children: translationsEn["pptx.media.title"] });
  }
...
function getTextStyleForElement(element, fallbackColor) {
  return buildTextBlockStyle(element, { fallbackColor });
}
...
function getTextLayoutStyle(element) {
  ...
  const shapeAutoFitTextBox = element.textStyle?.autoFitMode === "shrink" && (element.type === "text" || element.locks?.txBox === true);
  ...
  return {
    ...
    textIndent,
    ...shapeAutoFitTextBox ? {
      width: textWrapNone ? "max-content" : "100%",
      minWidth: "100%",
      height: "max-content",
      minHeight: "100%",
      whiteSpace: "pre-wrap",
      wordBreak: textWrapNone ? "normal" : "break-word",
      overflow: "visible"
    } : {},
    ...textWrapNone ? {
      whiteSpace: "nowrap",
...
function buildTextBody3DSceneStyle2(textStyle, elementSize) {
  return buildTextBody3DSceneStyle(textStyle, elementSize);
}

// src/viewer/utils/text-warp-css.tsx
function getTextWarpStyle(textStyle) {
...
function wrapWithTextBuildAnimation(elementId, paraIndex, renderedSegments, paraRuns, subElementAnimStates, renderPiece) {
  ...
  if (paraState) {
    ...
    return /* @__PURE__ */ jsx("span", { "data-anim-id": paraKey, style: { display: "inline", ...style }, children: renderedSegments }, paraKey);
  }
  ...
  return spec.spans.map((span, index) => {
    ...
    if (!span.animId) {
      return /* @__PURE__ */ jsx(React7.Fragment, { children: content }, `ws-${index}`);
    }
    return /* @__PURE__ */ jsx(
      "span",
      {
        "data-anim-id": span.animId,
        style: textBuildSpanStyle(span),
        children: content
      },
      span.animId
    );
...
function renderUnderlineWords(text, isUnderlineWords, wordDecoration, metric, keyPrefix, inner) {
  ...
  const gapMetric = metric ? {
    ...metric,
    nestedStyle: metric.nestedStyle ? stripUnderlineDecoration(metric.nestedStyle) : void 0
  } : void 0;
  return pieces.map((piece, i) => {
    const key = `${keyPrefix}-u${i}`;
    if (!piece.underline) {
      return /* @__PURE__ */ jsx(React7.Fragment, { children: inner(piece.text, key, gapMetric) }, key);
    }
    return /* @__PURE__ */ jsx("span", { style: wordDecoration, children: inner(piece.text, key, metric) }, key);
  });
}
function makeSegmentPieceRenderer(needsScriptFonts, scriptFonts, baseFontFamily, isUnderlineWords, wordDecoration, metric) {
  return (text, key) => renderUnderlineWords(
    text,
    isUnderlineWords,
    wordDecoration,
    metric,
    key,
    (subText, subKey, subMetric) => renderMetricPieces(
      subText,
      subMetric,
      subKey,
      (pieceText, pieceKey) => renderScriptAwareText(
        pieceText,
        needsScriptFonts,
        scriptFonts,
        baseFontFamily,
...
function renderTabbedLine(line, ctx, keyPrefix, renderPiece, nestedStyle, underlineWords = false) {
  ...
  return /* @__PURE__ */ jsx("span", { style: { display: "inline-block", whiteSpace: "nowrap" }, children: lineLayout.pieces.map((piece, i) => {
    const key = `${keyPrefix}-tab-${i}`;
    return /* @__PURE__ */ jsxs(React7.Fragment, { children: [
      piece.leaderStyle ? /* @__PURE__ */ jsx("span", { "aria-hidden": "true", style: piece.leaderStyle, children: piece.leaderText }) : null,
      piece.words ? piece.words.map((word, w) => /* @__PURE__ */ jsx("span", { style: word.style, children: renderPiece(word.text, `${key}-w${w}`) }, `${key}-w${w}`)) : /* @__PURE__ */ jsx("span", { style: piece.style, children: renderPiece(piece.text, key) })
    ] }, key);
  }) });
}
function renderScriptAwareText(text, needsScriptFonts, scriptFonts, baseFontFamily, keyPrefix, nestedStyle) {
  ...
  return pieces.map(
    (piece, i) => piece.style ? /* @__PURE__ */ jsx("span", { style: piece.style, children: piece.text }, `${keyPrefix}-r${i}`) : /* @__PURE__ */ jsx(React7.Fragment, { children: piece.text }, `${keyPrefix}-r${i}`)
  );
}
function renderMetricPieces(text, metric, keyPrefix, inner) {
  if (!metric || !text) {
  ...
  return pieces.map((piece, i) => /* @__PURE__ */ jsx(
    "span",
    {
      style: {
        ...metric.nestedStyle,
...
function renderSegmentContent(elementId, segmentIndex, textValue, lines, needsScriptFonts, scriptFonts, baseFontFamily, findHighlights, tabContext, metric, isUnderlineWords = false, wordDecoration) {
  ...
  if (!segHl || segHl.length === 0) {
    return lines.map((line, lineIndex) => {
      ...
      return /* @__PURE__ */ jsxs(React7.Fragment, { children: [
        tabContext && line.includes("	") ? renderTabbedLine(
          line,
          tabContext,
          lineKey,
          renderLeaf,
        ...
        lineIndex < lines.length - 1 ? /* @__PURE__ */ jsx("br", {}) : null
      ] }, lineKey);
    });
  }
  ...
  return chunks.map((chunk, ci) => {
    ...
    return chunk.highlighted ? /* @__PURE__ */ jsx(
      "mark",
      {
        style: {
          ...metric?.nestedStyle,
        ...
        children: renderLeaf(chunk.text, chunkKey)
      },
      chunkKey
    ) : /* @__PURE__ */ jsx(React7.Fragment, { children: renderLeaf(chunk.text, chunkKey) }, chunkKey);
  });
}
function renderEquationSegment(elementId, segmentIndex, equationXml, equationNumber) {
  ...
  ) : /* @__PURE__ */ jsx("span", { className: "inline-block px-1 py-0.5 rounded text-xs bg-gray-200/20 text-gray-400 italic", children: translationsEn["pptx.textSegment.equationFallback"] });
  if (equationNumber) {
    return /* @__PURE__ */ jsxs(
      "span",
      {
        ...
        children: [
          /* @__PURE__ */ jsxs("span", { style: { visibility: "hidden", whiteSpace: "nowrap" }, children: [
            "(",
            equationNumber,
            ")"
          ] }),
          /* @__PURE__ */ jsx("span", { style: { textAlign: "center", flex: 1 }, children: equationContent }),
          /* @__PURE__ */ jsxs(
            "span",
            {
              style: {
...
function wrapWithTextReflection(run, key, spanNode, spanStyle, baseContent) {
  if (!run.reflection) {
    return spanNode;
  }
  return /* @__PURE__ */ jsxs("span", { style: { position: "relative", display: "inline-block" }, children: [
    ...
    /* @__PURE__ */ jsx(
      "span",
      {
        ...
        style: run.reflection,
        children: /* @__PURE__ */ jsx("span", { style: spanStyle, children: baseContent })
      }
    )
  ] }, `${key}-reflwrap`);
}
...
function renderParagraphRun(run, segment, ctx, keySuffix = "") {
  ...
  const fontKerning = resolveFontKerning(segmentStyle.kerning, baseFontSize);
  ...
  const nestedStyle = nestedTextDecorationStyle(run.style);
  ...
  const hollow = hollowTextFillStyle(segmentStyle, {
    ...
    textStroke: typeof spanStyle.WebkitTextStroke === "string" ? spanStyle.WebkitTextStroke : void 0
  });
  if (hollow) {
  ...
  const isUnderlineWords = Boolean(segmentStyle.underline) && segmentStyle.underlineStyle === "words";
  const wordDecoration = isUnderlineWords ? nestedStyle ?? run.underlineWordPieces?.find((piece) => piece.style)?.style : void 0;
  if (isUnderlineWords) {
    const stripped = stripUnderlineDecoration(spanStyle);
    spanStyle.textDecoration = stripped.textDecoration;
    delete spanStyle.textDecorationStyle;
    delete spanStyle.textDecorationThickness;
    delete spanStyle.textUnderlineOffset;
  ...
  return run.hyperlink && onHyperlinkClick ? renderHyperlink(run, reflectedNode, key, ctx) : reflectedNode;
}
...
function renderTextSegments(element, fallbackColor, emptyFallback, findHighlights, onHyperlinkClick, fieldContext, subElementAnimStates, segmentOverrides, requireCtrlClick, placeholderPrompt) {
  if (!hasTextProperties(element)) {
  ...
  if (!segments || segments.length === 0) {
    if (placeholderPrompt) {
      return /* @__PURE__ */ jsx("span", { style: placeholderPrompt.style, children: placeholderPrompt.text });
    }
...
function renderDagDuotoneSvgFilter(elementId, color1, color2) {
  ...
  return /* @__PURE__ */ jsx(
    "svg",
    {
      ...
      "aria-hidden": "true",
      children: /* @__PURE__ */ jsx("defs", { children: /* @__PURE__ */ jsxs("filter", { id: filterId, colorInterpolationFilters: "sRGB", children: [
        /* @__PURE__ */ jsx("feColorMatrix", { type: "matrix", values: grayscaleMatrix }),
        /* @__PURE__ */ jsxs("feComponentTransfer", { children: [
          /* @__PURE__ */ jsx("feFuncR", { type: "linear", slope: c2.r - c1.r, intercept: c1.r }),
          /* @__PURE__ */ jsx("feFuncG", { type: "linear", slope: c2.g - c1.g, intercept: c1.g }),
          /* @__PURE__ */ jsx("feFuncB", { type: "linear", slope: c2.b - c1.b, intercept: c1.b })
        ] })
      ] }) })
    }
  );
...
function apply3dEffects2(base, scene3d, shape3d, fillColorFallback, elementSize) {
  apply3dEffects(base, scene3d, shape3d, fillColorFallback, elementSize);
}
...
function rendersWedgeCalloutVector(element) {
...
function getShapeVisualStyle(element, _hasFill, fillColor, _strokeWidth, _strokeColor, animatesFill, animatesStroke, parentGroupFill) {
  if (element.type === "group") {
    const fx2 = getComputedEffectStyle(element);
    const groupStyle = {};
    if (fx2.filter) {
      groupStyle.filter = fx2.filter;
    }
    if (fx2.overflowVisible) {
  ...
  if (element.type === "image" || element.type === "picture") {
    base.borderWidth = 0;
    base.borderColor = void 0;
    base.borderStyle = void 0;
  }
  apply3dEffects2(base, ss?.scene3d, ss?.shape3d, ss?.fillColor ?? fillColor, {
    width: element.width,
    height: element.height
  });
  if (rendersCustomVectorPath(element) || rendersWedgeCalloutVector(element) || isStrokeOnlyPresetElement(element)) {
...
var ARROW_SIZE_SCALE = {
  sm: 0.6,
  med: 1,
  lg: 1.5
};
function renderConnectorMarker(markerId, arrowType, color, arrowWidth, arrowLength) {
  ...
  return /* @__PURE__ */ jsx(
    "marker",
    {
      ...
      children: arrowType === "triangle" ? /* @__PURE__ */ jsx("polygon", { points: "0,0 10,5 0,10", fill: color }) : arrowType === "stealth" ? /* @__PURE__ */ jsx("polygon", { points: "0,0 10,5 0,10 3.4,5", fill: color }) : arrowType === "diamond" ? /* @__PURE__ */ jsx("polygon", { points: "0,5 5,0 10,5 5,10", fill: color }) : arrowType === "oval" ? /* @__PURE__ */ jsx("ellipse", { cx: 5, cy: 5, rx: 4, ry: 3.3, fill: color }) : /* @__PURE__ */ jsx("path", { d: "M0 0 L10 5 L0 10", fill: "none", stroke: color, strokeWidth: 1.6 })
    }
  );
}
function renderSvgGradientDefs(gradient) {
  if (!gradient) {
    return null;
  }
  if (gradient.kind === "rectPath") {
    return /* @__PURE__ */ jsx("defs", { children: /* @__PURE__ */ jsx("pattern", { id: gradient.id, patternUnits: "objectBoundingBox", width: 1, height: 1, children: /* @__PURE__ */ jsx("image", { href: gradient.href, x: 0, y: 0, width: 1, height: 1, preserveAspectRatio: "none" }) }) });
  }
  const stops = gradient.stops.map((stop, index) => /* @__PURE__ */ jsx(
    "stop",
    {
      offset: stop.offset,
      stopColor: stop.color,
      stopOpacity: typeof stop.opacity === "number" ? stop.opacity : void 0
    },
    index
  ));
  return /* @__PURE__ */ jsx("defs", { children: gradient.kind === "radial" ? /* @__PURE__ */ jsx("radialGradient", { id: gradient.id, cx: gradient.cx, cy: gradient.cy, r: gradient.r, children: stops }) : /* @__PURE__ */ jsx(
    "linearGradient",
    {
      id: gradient.id,
      x1: gradient.x1,
...
function joinStyle(shapeStyle) {
  const lineJoin = shapeStyle?.lineJoin === "bevel" ? "bevel" : shapeStyle?.lineJoin === "miter" ? "miter" : "round";
  const miterLimit = lineJoin === "miter" && typeof shapeStyle?.miterLimit === "number" ? Math.max(shapeStyle.miterLimit / 1e5, 1) : void 0;
  return { lineJoin, miterLimit };
}
function strokeContext(shapeStyle, strokePaint, strokeWidth, dashArray) {
...
function strokeStrands(d, keyBase, ctx) {
  return ctx.offsets.map((offset, idx) => /* @__PURE__ */ jsx(
    "path",
    {
      d,
      fill: "none",
      ...
      style: offset !== 0 ? { transform: `translate(0, ${offset}px)` } : void 0
    },
    `${keyBase}-s${idx}`
  ));
}
function renderCustomGeometryVector(pathData, pathWidth, pathHeight, structuredPaths, shapeStyle, hasFill, fillColor, fillOpacity, strokePaint, strokeWidth, dashArray, animatesFill = false, gradient) {
  ...
  const subpaths = structuredPaths && structuredPaths.length > 0 ? customGeometryPathsToSvgSubpaths(structuredPaths, pathWidth, pathHeight) : void 0;
  const needsPerSubpath = subpaths !== void 0 && subpaths.some((sp) => sp.fillMode && sp.fillMode !== "norm" || sp.stroke === false);
  const gradientPaint = gradient ? svgGradientFillRef(gradient) : void 0;
  const nodes = [];
  if (subpaths && needsPerSubpath) {
    ...
    paints.forEach((paint, idx) => {
      if (paint.fill !== "none") {
        ...
        const useGradient = gradientPaint && (mode === void 0 || mode === "norm");
        nodes.push(
          /* @__PURE__ */ jsx(
            "path",
            {
    ...
    if (hasFill) {
      nodes.push(
        /* @__PURE__ */ jsx(
          ...
          {
            d: pathData,
            fill: animatesFill ? "inherit" : gradientPaint ?? colorWithOpacity(fillColor, fillOpacity),
            stroke: "none",
            vectorEffect: "non-scaling-stroke"
          },
          "fill"
...
function renderVectorShape(element, hasFill, fillColor, strokeWidth, strokeColor, animatesFill, animatesStroke) {
  ...
  const normalizedType = (element.shapeType || "").toLowerCase();
  const fillPaint = animatesFill ? "inherit" : colorWithOpacity(fillColor, element.shapeStyle?.fillOpacity);
  const strokePaint = animatesStroke ? "inherit" : colorWithOpacity(strokeColor, element.shapeStyle?.strokeOpacity);
  ...
  if ((element.type === "shape" || element.type === "image" || element.type === "picture") && element.pathData && element.pathWidth && element.pathHeight && element.pathWidth > 0 && element.pathHeight > 0) {
  ...
  if ((element.type === "image" || element.type === "picture") && strokeWidth > 0 && !buildStrokeOutline(element)) {
    const width = Math.max(element.width, 1);
    const height = Math.max(element.height, 1);
    ...
    if (geometry && geometry.points.length >= 2) {
      ...
      return /* @__PURE__ */ jsxs(
        "svg",
        {
          ...
          children: [
            geometry.hasAccent && /* @__PURE__ */ jsx(
              "line",
              {
                x1: 0,
                y1: 0,
  ...
  if (element.type === "connector" || getShapeType(element.shapeType) === "connector" || element.shapeType === "line") {
    ...
    return /* @__PURE__ */ jsxs(
      "svg",
      {
        ...
        children: [
          ...
          offsets.map((offset, idx) => /* @__PURE__ */ jsx(
            "path",
            {
              d: pathData,
              fill: "none",
              ...
              strokeLinejoin: "round",
              markerStart: idx === 0 && startArrow && startArrow !== "none" ? `url(#${startMarkerId})` : void 0,
              markerEnd: idx === offsets.length - 1 && endArrow && endArrow !== "none" ? `url(#${endMarkerId})` : void 0,
              vectorEffect: "non-scaling-stroke",
              style: {
                pointerEvents: "none",
...
// src/viewer/utils/table-cell-fill.tsx
function parseGradientFillCss(gradFill) {
  if (!gradFill) {
    return void 0;
  }
  const gsLstRaw = gradFill["a:gsLst"];
  if (!gsLstRaw) {
    return void 0;
  }
  const stops = ensureArrayValue(gsLstRaw["a:gs"]);
  if (stops.length === 0) {
    return void 0;
  }
  const rawStops = stops.map((gs) => {
    const pos = Number.parseInt(String(gs["@_pos"] || "0"), 10) / 1e3;
    const color = parseDrawingColor(gs) ?? "#888888";
    const opacity = parseDrawingColorOpacity(gs);
    return { position: pos, color, opacity };
  }).sort((a, b) => a.position - b.position);
  const stopStrings = rawStops.map((s) => `${colorWithOpacity(s.color, s.opacity)} ${s.position.toFixed(1)}%`).join(", ");
  const lin = gradFill["a:lin"];
  if (lin) {
    const angRaw = Number.parseInt(String(lin["@_ang"] || "0"), 10);
    const angleDeg = Math.round(ooxmlGradientAngleToCssDegrees(angRaw / 6e4));
    return `linear-gradient(${angleDeg}deg, ${stopStrings})`;
  }
  const path = gradFill["a:path"];
  if (path) {
    const pathType = String(path["@_path"] || "circle");
    const fillToRectXml = path["a:fillToRect"];
    const fillToRect = fillToRectXml ? {
      l: Number.parseInt(String(fillToRectXml["@_l"] || "0"), 10) / 1e5,
      t: Number.parseInt(String(fillToRectXml["@_t"] || "0"), 10) / 1e5,
      r: Number.parseInt(String(fillToRectXml["@_r"] || "0"), 10) / 1e5,
      b: Number.parseInt(String(fillToRectXml["@_b"] || "0"), 10) / 1e5
    } : void 0;
    if (pathType === "rect") {
      return buildRectPathGradient(rawStops, void 0, fillToRect);
    }
    let cx = 50;
    let cy = 50;
    if (fillToRect) {
...
function parsePatternFillCss(pattFill) {
  if (!pattFill) {
    return void 0;
  }
  const fgColor = parseDrawingColor(pattFill["a:fgClr"]) ?? "#000000";
  const bgColor = parseDrawingColor(pattFill["a:bgClr"]) ?? "#ffffff";
...
function ooxmlDashToCss(dashVal) {
  if (!dashVal) {
    return "solid";
  }
  switch (dashVal) {
...
function parseCellBorders(cellProperties) {
  if (!cellProperties) {
    return {};
  }
  const borderStyle = {};
  const edges = [
    { xml: "a:lnL", prefix: "borderLeft" },
    { xml: "a:lnR", prefix: "borderRight" },
    { xml: "a:lnT", prefix: "borderTop" },
    { xml: "a:lnB", prefix: "borderBottom" }
  ];
  for (const edge of edges) {
    const ln = cellProperties[edge.xml];
    if (!ln) {
      continue;
    }
    ...
    const color = parseDrawingColor(ln["a:solidFill"]);
    if (color) {
      const opacity = parseDrawingColorOpacity(ln["a:solidFill"]);
      const key = `${edge.prefix}`;
      borderStyle[key] = `${widthPx}px ${cssBorderStyle} ${colorWithOpacity(color, opacity)}`;
    }
  }
  return borderStyle;
}
function parseCellTextEffects(runProps) {
  const effectLst = runProps?.["a:effectLst"];
  if (!effectLst) {
    return void 0;
  }
  ...
  if (outerShdw) {
    ...
    if (shdwColor) {
      ...
      let offY = 0;
      if (distVal > 0) {
        const angleRad = dirVal / 6e4 * (Math.PI / 180);
  ...
  return textShadowParts.length > 0 ? textShadowParts.join(", ") : void 0;
}

// src/viewer/utils/table-cell-style.tsx
function extractCellText(cellXml) {
  ...
  paragraphs.forEach((paragraph) => {
    const textParts = [];
    const runs = ensureArrayValue(paragraph["a:r"]);
    runs.forEach((run) => {
      const value = run["a:t"];
    ...
    const fields = ensureArrayValue(paragraph["a:fld"]);
...
function extractTableCellStyle(cellXml, fallbackStyle, schemeColorOverrides) {
  ...
  const cellStyle = { ...fallbackWithoutColor };
  const txBody = cellXml["a:txBody"];
  const paragraphs = ensureArrayValue(txBody?.["a:p"]);
  const firstParagraph = paragraphs[0];
  const paragraphProperties = firstParagraph?.["a:pPr"];
  const runs = ensureArrayValue(firstParagraph?.["a:r"]);
  const firstRun = runs[0];
  const runProps = firstRun?.["a:rPr"] || paragraphProperties?.["a:defRPr"];
  if (runProps?.["@_sz"]) {
  ...
  if (runProps?.["@_b"] !== void 0) {
    cellStyle.fontWeight = runProps["@_b"] === "1" ? 700 : 400;
  }
  if (runProps?.["@_i"] !== void 0) {
  ...
  const runColor = parseDrawingColor(runProps?.["a:solidFill"], schemeColorOverrides) || parseDrawingColor(runProps?.["a:highlight"], schemeColorOverrides);
  if (runColor) {
    const opacity = parseDrawingColorOpacity(runProps?.["a:solidFill"]);
  ...
  if (cellFill) {
    ...
    if (gradCss) {
      ...
      const pattCss = parsePatternFillCss(cellProperties?.["a:pattFill"]);
      if (pattCss) {
  ...
  if (!cellStyle.borderLeftColor && !cellStyle.borderRightColor && !cellStyle.borderTopColor && !cellStyle.borderBottomColor) {
    const borderColor = parseDrawingColor(
  ...
  if (cellProperties) {
    ...
    const toPx = (emu) => Math.round(emu / 12700);
...
function getTableCellBandStyle2(element, rowIndex, cellIndex, rowCount, columnCount, styleCtx) {
  ...
  return getTableCellBandStyle(element.tableData, rowIndex, cellIndex, rowCount, columnCount, {
...
function getGraphicDataFromElement(element) {
  return element.rawXml?.["a:graphic"]?.["a:graphicData"];
}
function getTableXmlFromElement(element) {
  return getGraphicDataFromElement(element)?.["a:tbl"];
}
function parseTableElementData(element, fallbackTextStyle) {
  const table = getTableXmlFromElement(element);
  if (!table) {
    return null;
  }
  const rows = ensureArrayValue(table["a:tr"]);
  if (rows.length === 0) {
    return null;
  }
  const tableGrid = table["a:tblGrid"];
  ...
  const columnPercentages = totalColumnWidth > 0 ? columnWidths.map((width) => width / totalColumnWidth * 100) : [];
  const cells = [];
  rows.forEach((row, rowIndex) => {
    const rowCells = ensureArrayValue(row["a:tc"]);
...
function TableCellDiagonalBorders({
  diag
}) {
...
function TableResizeOverlay({
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      drag.handleEl.style.transform = drag.type === "col" ? `translateX(${delta}px)` : `translateY(${delta}px)`;
    };
  ...
  const startRowDrag = (e, index) => {
    ...
    const actualHeight = tr?.offsetHeight ?? DEFAULT_ROW_HEIGHT;
  ...
  return /* @__PURE__ */ jsxs("div", { ref: containerRef, className: "relative w-full h-full", children: [
    children,
    colBoundaries.map((leftPct, i) => /* @__PURE__ */ jsx(
      "div",
      {
        ...
        children: /* @__PURE__ */ jsx("div", { className: "w-px h-full mx-auto bg-transparent group-hover:bg-blue-400/60 transition-colors" })
      },
      `col-h-${i}`
    )),
    rowBounds.map((topPx, i) => /* @__PURE__ */ jsx(
      "div",
      {
        ...
        children: /* @__PURE__ */ jsx("div", { className: "h-px w-full my-auto bg-transparent group-hover:bg-blue-400/60 transition-colors" })
      },
      `row-h-${i}`
    ))
  ] });
}
function renderTableFromTableData(element, textStyle, options) {
          ...
          children: /* @__PURE__ */ jsxs(
            "table",
            {
              ...
              children: [
                ...
                /* @__PURE__ */ jsx("tbody", { children: tableData.rows.map((row, rowIndex) => /* @__PURE__ */ jsx(
                  "tr",
                  {
                    style: row.height ? { height: row.height } : void 0,
                      ...
                      return /* @__PURE__ */ jsxs(
                        "td",
                        {
                          className: cn(
                            ...
                            isEditable && hasCellSelectionHandler ? "border-blue-200/70 cursor-cell" : "border-gray-400/50",
                            isCellSelected ? "ring-1 ring-inset ring-blue-500" : null,
                            isInMultiSelection && !isCellSelected ? "bg-blue-500/15 ring-1 ring-inset ring-blue-400/50" : null
                          ),
                          ...
                          children: [
                            diag ? /* @__PURE__ */ jsx(TableCellDiagonalBorders, { diag }) : null,
                            isCellEditing ? /* @__PURE__ */ jsx(
                              TableCellInput,
                              {
...
function renderTableElement(element, textStyle, options) {
  const parsedTable = parseTableElementData(element, textStyle);
  if (!parsedTable) {
    const tableEl2 = element;
    if (tableEl2.tableData && tableEl2.tableData.rows.length > 0) {
      return renderTableFromTableData(tableEl2, textStyle, options);
    }
    return /* @__PURE__ */ jsx("div", { className: "w-full h-full flex items-center justify-center text-[11px] text-white/80 pointer-events-none", children: translationsEn["pptx.table.title"] });
  }
  const selectedCell = options?.selectedCell || null;
  const isEditable = Boolean(options?.editable) && canDrillDown(element);
  const hasCellSelectionHandler = typeof options?.onSelectCell === "function";
  const tableEl = element;
  const xmlSelectionRect = (() => {
    if (!selectedCell?.selectedCells || selectedCell.selectedCells.length < 2) {
      return void 0;
    }
            ...
            "table",
            {
              ...
              children: [
                parsedTable.columnPercentages.length > 0 ? /* @__PURE__ */ jsx("colgroup", { children: parsedTable.columnPercentages.map((percentage, columnIndex) => /* @__PURE__ */ jsx(
                  ...
                  `${element.id}-col-${columnIndex}`
                )) }) : null,
                /* @__PURE__ */ jsx("tbody", { children: parsedTable.rows.map((row, rowIndex) => {
                  const cells = ensureArrayValue(row["a:tc"]);
                  const rowHeightRaw = Number.parseInt(String(row["@_h"] || ""), 10);
                  const rowHeight = Number.isFinite(rowHeightRaw) && rowHeightRaw > 0 ? Math.max(16, rowHeightRaw / EMU_PER_PX) : void 0;
                  return /* @__PURE__ */ jsx(
                    "tr",
                    {
                        ...
                        return /* @__PURE__ */ jsxs(
                          "td",
                          {
                            className: cn(
                              ...
                              isCellInRect(rowIndex, cellIndex, xmlSelectionRect) && !(selectedCell?.rowIndex === rowIndex && selectedCell?.columnIndex === cellIndex) ? "bg-blue-500/15 ring-1 ring-inset ring-blue-400/50" : null
                            ),
                            colSpan,
                            rowSpan: rSpan,
                            style: mergedStyle,
                            ...
                            children: [
                              diag ? /* @__PURE__ */ jsx(TableCellDiagonalBorders, { diag }) : null,
                              selectedCell?.rowIndex === rowIndex && selectedCell?.columnIndex === cellIndex && selectedCell?.isEditing ? /* @__PURE__ */ jsx(
                                TableCellInput,
                                {
...
function partAttrs(part) {
  return part ? chartPartToAttrs(part) : void 0;
}
...
function buildReactChartViewModel(element) {
  ...
  const palette = resolveReactPalette(element.chartData), themedElement = {
    ...element,
    ...
    case "rect": {
      ...
      return /* @__PURE__ */ jsx(
        "rect",
        {
          ...
          opacity: r.opacity ?? 1,
          ...partAttrs(r.part),
          children: r.title !== void 0 ? /* @__PURE__ */ jsx("title", { children: r.title }) : null
        },
        key
      );
    }
    ...
    case "circle": {
      ...
      return /* @__PURE__ */ jsx(
        "circle",
        {
          ...
          opacity: c.opacity ?? 1,
          ...partAttrs(c.part),
          children: c.title !== void 0 ? /* @__PURE__ */ jsx("title", { children: c.title }) : null
        },
        key
      );
    }
    case "line": {
      ...
      return /* @__PURE__ */ jsx(
        "line",
        {
          ...
          children: l.title !== void 0 ? /* @__PURE__ */ jsx("title", { children: l.title }) : null
        },
        key
      );
    }
...
function renderChartViewModel(elementId, vm, preserveAspectRatio = "none") {
  ...
  return /* @__PURE__ */ jsxs(
    "svg",
    {
      ...
      preserveAspectRatio,
      children: [
        vm.defs && vm.defs.length > 0 && /* @__PURE__ */ jsx("defs", { children: vm.defs.map((def, i) => /* @__PURE__ */ jsx(
          "pattern",
          {
            id: def.id,
        ...
        vm.title && /* @__PURE__ */ jsx(
          "text",
          {
            ...
            children: vm.titleRunSpans && vm.titleRunSpans.length > 0 ? vm.titleRunSpans.map((run, i) => /* @__PURE__ */ jsx(
              "tspan",
              {
              ...
              `${elementId}-title-run-${i}`
            )) : vm.title
          }
        ),
        vm.gridlines.map((gl, i) => /* @__PURE__ */ jsx(
          "line",
          {
            x1: gl.x1,
            y1: gl.y1,
        ...
        (vm.secondaryGridlines ?? []).map((gl, i) => /* @__PURE__ */ jsx(
          "line",
          {
            x1: gl.x1,
            y1: gl.y1,
        ...
        legendItems.map((item, i) => /* @__PURE__ */ jsxs(
          "g",
          {
            transform: `translate(${item.x.toFixed(1)},${item.y.toFixed(1)})`,
            children: [
              /* @__PURE__ */ jsx("rect", { x: 0, y: -7, width: 10, height: 10, rx: 2, fill: item.color }),
              /* @__PURE__ */ jsx(
                "text",
                {
                  x: 13,
...
function buildNotesPdf(pages) {
  const images = pages.map((page) => {
    ...
    for (let i = 0; i < raw.length; i++) {
      bytes2[i] = raw.charCodeAt(i);
    }
    return { bytes: bytes2, w: page.canvas.width, h: page.canvas.height };
  });
  const meta = pages.map((page) => ({
    notes: page.notes,
...
function startMotionPathPreview(elementId, path, options) {
  stopPreviewAnimation();
...
var DEFAULT_PREVIEW_SLIDE_WIDTH = 1280;
var DEFAULT_PREVIEW_SLIDE_HEIGHT = 720;
function playDescriptor(elementId, descriptor) {
  const domEl = document.querySelector(`[data-element-id="${elementId}"]`);
  if (!domEl) {
    return;
  }
...
export { ANIMATION_PRESET_OPTIONS, ARROW_SIZE_OPTIONS, CONNECTOR_ARROW_OPTIONS, DEFAULT_CANVAS_HEIGHT, DEFAULT_CANVAS_WIDTH, DEFAULT_FILL_COLOR, DEFAULT_FONT_FAMILY2 as DEFAULT_FONT_FAMILY, DEFAULT_SECTION_GROUP_ID, DEFAULT_STROKE_COLOR, DEFAULT_TABLE_COLUMNS, DEFAULT_TABLE_ROWS, DEFAULT_TEXT_COLOR, DEFAULT_TEXT_FONT_SIZE, GRID_SIZE, HYPERLINK_COLOR, MIN_ELEMENT_SIZE, SHAPE_PRESETS, SHORTCUT_REFERENCE_ITEMS, SLIDE_NAV_THUMBNAIL_WIDTH, STROKE_DASH_OPTIONS2 as STROKE_DASH_OPTIONS, TOOLBAR_SECTIONS, UNGROUPED_SECTION_ID, ZOOM_TO_SELECTION_PADDING, build3DExtrusionData2 as build3DExtrusionData, buildLineGlowFilter, buildLineShadowCss, buildNotesPdf, buildPdfFromImageData, buildReactChartViewModel, buildTextBody3DSceneStyle2 as buildTextBody3DSceneStyle, cloneHistorySnapshot2 as cloneHistorySnapshot, cn, getAnimationInitialStyle2 as getAnimationInitialStyle, getConnectorPathGeometry, getCropShapeClipPath2 as getCropShapeClipPath, getDuotoneColors, getImageRenderStyle, getImageSurfaceMaskStyle, getImageSurfaceStyle, getImageTilingStyle2 as getImageTilingStyle, getShapeVisualStyle, getTextLayoutStyle, getTextStyleForElement, getTextWarpStyle, hasDagDuotoneEffect, isConnectorOrLineElement, renderChartElement, renderConnectorMarker, renderDagDuotoneSvgFilter, renderMediaElement, renderTableElement, renderTextSegments, renderVectorShape, shouldRenderFallbackLabel, startMotionPathPreview, startPreviewAnimation, stopPreviewAnimation };
